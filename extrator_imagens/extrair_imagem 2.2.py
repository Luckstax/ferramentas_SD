...
class App(ctk.CTk):
    def __init__(self):
        ...
        self.sim_threshold = ctk.DoubleVar(value=0.85) # Threshold padrão
    ...
    def setup_ui(self):
        ...
        # Slider de Threshold
        ctk.CTkLabel(self.frame_files, text="Sensibilidade (Threshold):").grid(row=4, column=0, sticky="w", padx=10, pady=(10,0))
        self.slider = ctk.CTkSlider(self.frame_files, from_=0.5, to=1.0, variable=self.sim_threshold, number_of_steps=50)
    ...
    def process_video(self):
        try:
            # Configurações Dinâmicas e Internas
            FRAME_SKIP = 5
            SIMILARITY_THRESHOLD = self.sim_threshold.get()
            ...
            if not base_embeddings:
                raise ValueError("Nenhuma imagem válida (.jpg, .png) encontrada na pasta base.")
